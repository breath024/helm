...
12. **세션 목록의 "갈라진 세션"** — 세션이 갈라지면 CLI 가 옛 파일 끝에
   `{"type":"continued-in","continuedInSessionId":"<새 id>"}` 를 남기는데,
   **갈라진 쪽이 앞부분을 통째로 복사해간다.** 제목(첫 사용자 발화)도 프로젝트도 mtime 도
   원본과 같아서 목록에서 구분이 안 된다. 버려진 쪽을 이어받으면 그 뒤 작업이 사라진
   것처럼 보인다 — 2026-09-21 에 호윤이 하루에 두 번 밟았다.
   - 지금은 `scan_sessions()` 가 **갈라진 뒤 대화가 0 인 가지를 목록에서 뺀다.**
   - 양쪽 다 계속 쓴 진짜 분기는 둘 다 남기고 `⑂ 갈라짐` 배지를 붙인다.
   - ⚠ **배지 판정에 `continued-in` 을 쓰지 말 것.** 갈라진 뒤 많이 쓴 세션은 그 자취가
     `_tail_scan` 의 64KB 창 밖으로 밀려나 안 잡힌다. 배지는 *제목+프로젝트 중복* 으로 본다.
     숨기기 쪽은 마커로 해도 된다(버려진 가지는 마커가 파일 끝이라 항상 창 안).
13. **`_head_scan`·`_tail_scan` 이 읽는 항목을 늘리면 `_CACHE_VER` 를 올릴 것** —
   `.session_cache.json` 은 파일의 mtime·size 가 그대로면 다시 안 읽는다. 옛 세션은
   파일이 영영 안 변하므로 **새 항목이 영원히 빈 채로 남는다.** 12번 고칠 때 실제로 밟았다.
...
## 기록
- [2026-09-21](logs/2026-09-21-log.md) — 갈라진 세션이 목록에서 구분 안 되던 것 (버려진 가지를 열어 작업이 사라진 줄 앎) + 9/15 hcom 작업분 커밋
